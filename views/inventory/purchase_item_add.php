  ...
  function updatePurchaseLanguage(language) {
    ...
    productItems.forEach(item => {
      ...
      dropdownItems.forEach((dropdownItem, index) => {
        const textSpan = dropdownItem.querySelector('span') || dropdownItem;
        if (index < purchaseTranslations[language].dropdown_options.length) {
          const icon = dropdownItem.querySelector('i').outerHTML; // Preserve icon
          textSpan.innerHTML = `${icon} ${purchaseTranslations[language].dropdown_options[index]}`;
        }
      });
    });
